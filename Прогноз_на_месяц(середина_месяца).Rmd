...
date_finish <- as_datetime(today())
date_finish_for_base <- date_finish + years(2000)

date_start  <- date_finish - months(36)
...
transaction_log_clv_month <- transaction_log %>%
    group_by(cust) %>% 
    mutate(first_sale_date = min(date)) %>% 
    filter(first_sale_date < date_finish - months(1)) %>% 
    ungroup()

clv_df_month <- clvdata(transaction_log_clv_month,
                  date.format="ymd",
                  time.unit = "week",
...
staticCov_month <- transaction_log_clv_month %>% 
...
clv_static_month <- SetStaticCovariates(clv.data = clv_df_month,
                                 data.cov.life = staticCov_month,
                                 data.cov.trans = staticCov_month,
                                 names.cov.life = c("type", "registr_date"),
                                 names.cov.trans = c("type", "registr_date"),
                                 name.id = "cust")

estimation_month <- bgnbd(clv_static_month,
                    start.params.model = c(r=1, alpha=3, a =1, b=5),
                    reg.lambdas = c(trans=100, life=100)
                    #optimx.args = list(method="Nelder-Mead")
                          )
prediction_month <- predict(estimation_month, prediction.end = date_finish - days(1))
...
roc_tbl_raw <- prediction_month %>% 
    transmute(pred = ifelse(CET < 1, CET, 1),
              truth = ifelse(actual.x == 0, 0, 1))

threshold <- round(ROC_threshold(roc_tbl_raw$truth, roc_tbl_raw$pred)[1], 2) - 0.01
...
```{r}
conf_tbl <- prediction %>% 
    transmute(pred = ifelse(CET < threshold_month, 0, 1),
              truth = ifelse(actual.x == 0, 0, 1))
library(cvms)
...
pred_end_month <- ceiling_date(date_finish + months(1), unit = "month") - days(1)
                                                                      
prediction_new_month <- predict(estimation_new, prediction.end = pred_end_month)

pred_end_week <- ceiling_date(date_finish, unit = "month") - days(1)
                                                                      
prediction_new_week <- predict(estimation_new, prediction.end = pred_end_week)



rfm_results <- readxl::read_xlsx(paste0("RFM/RFM-анализ_за_",
                                        month(date_finish - months(1),
                                              label = TRUE, abbr = FALSE),
                                        ".xlsx")) %>% 
     mutate(user_name = str_trim(customer_name, side = "right"))

prediction_df <- prediction_new_month %>% 
    left_join(select(prediction_new_week, Id, week_CET = CET)) %>% 
    select(Id, predicted.mean.spending, CET, week_CET) %>% 
    separate(Id, into = c("customer_id", "subdiv_name"),
             sep = ";") %>%
    mutate(predicted.mean.spending = round(predicted.mean.spending),
           CET_new = CET - week_CET) %>% 
    # left_join(select(ref_customers, customer_id, customer_name)) %>%
    left_join(select(customers_tbl, customer_rfm_id, subdiv_new, main_flag),
    ...
    select(customer_id, subdiv_name, main_flag,
           predicted.mean.spending, CET_new) %>%
    mutate(sale_plan_qty = case_when(
                                     CET_new < threshold_month ~ 0,
                                     CET_new < 0.5 ~ 1,
                                     TRUE      ~ round(CET_new)),
           sale_plan_sum = round(sale_plan_qty * predicted.mean.spending * 1.5)) %>%
    select(-c(CET_new)) %>%
...
sum(prediction_df$sale_plan_sum)
