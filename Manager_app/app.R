...
predict_month <- readxl::read_xlsx(here(paste0("Планирование/Прогноз_продаж_",
                                        month(today(),
...
rfm_results <- readxl::read_xlsx(here(paste0("RFM/RFM-анализ_за_",
                                        month(today() - months(1),
                                              label = TRUE, abbr = FALSE),
                                        ".xlsx"))) %>% 
    mutate(user_name = str_trim(customer_name, side = "right"))

sale_df_previos <- read_rds(here(paste0("Data/sale_df_finish_",
                                          month(today() - months(1),
                                                label = TRUE),
                                          ".xlsx")))
...
managers <- predict_month %>% 
    ...
    distinct(customer_manager) %>% 
    filter(!is.na(customer_manager)) %>% 
    pull()

sale_data <- sale_data_global(floor_date(today(), unit = "month") + years(2000),
...
returns_data <- refund_data(floor_date(today(), unit = "month") + years(2000),
                            today() + years(2000) + days(1)) %>% 
    mutate(date_refund_doc = as.Date(date_refund_doc) - years(2000),
           date_sale_doc = as.Date(date_sale_doc) - years(2000)) %>% 
    filter(date_sale_doc >= floor_date(today(), unit = "month")) %>% 
...
supplier_order_df <- supplier_order_data(floor_date(today(),
                                                    unit="month") + years(2000),
                                         today() + years(2000) + days(1)) %>% 
    filter(!is.na(nmbr_cust_order_doc)) %>% 
    select(nmbr_supplier_order, supplier_order_doc_sum, nmbr_cust_order_doc,
           date_cust_order_doc) %>% 
    distinct(nmbr_supplier_order, .keep_all = TRUE) %>% 
    mutate(date_cust_order_doc = as.Date(date_cust_order_doc) - years(2000)) %>% 
    filter(date_cust_order_doc >= floor_date(today(), unit="month"))


customer_orders_df <- customer_order_data(today() - months(3) + years(2000),
                                      today() + years(2000) + days(1)) %>% 
    mutate(date_cust_order_doc = as.Date(date_cust_order_doc) - years(2000)) %>% 
...
debt_cust_mov_data <- customer_debt_moving_data(floor_date(today(),
                                                           unit = "month") + 
                                                           years(2000) -
                                                           years(1),
                                                       today() + years(2000) + 
            ...
            tabsetPanel(
               ...
               tabPanel(
                ...
                tabPanel(
                         title = "План на день",
                         column(width = 12,
                                dataTableOutput("day_plan_table", width="100%")
                                ),
...
server <- function(input, output) {
    ...
    output$debt_table <- renderDataTable({
        ...
        debt_tbl() %>%
            ...
            summarise(`Общая задол-ть` = round(first(debt_sum), 2),
                      value = round(sum(sale_doc_sum)/1000, 2)) %>% 
    ...
    output$day_plan_table <- renderDataTable({
